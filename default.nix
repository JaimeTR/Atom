...
in stdenv.mkDerivation rec {
  ...
  cmakeFlags = [
    "-DHAS_ATOMLANG_VERSION=0"
    "-DATOMLANG_VERSION=${version}"
    "-DATOMLANG_LUA=luajit"
    "-DCLANG_RESOURCE_DIR=${llvmMerged}/lib/clang/${clangVersion}"
  ] ++ lib.optional enableCUDA "-DATOMLANG_ENABLE_CUDA=ON";
  ...
  postPatch = ''
    ...
    substituteInPlace src/atomlanglib.lua \
  ...
  installPhase = ''
    install -Dm755 -t $bin/bin bin/atomlang
    install -Dm755 -t $out/lib lib/atomlang${stdenv.hostPlatform.extensions.sharedLibrary}
    install -Dm644 -t $static/lib lib/libatomlang_s.a

    mkdir -pv $dev/include
    cp -rv include/atomlang $dev/include
  '';

  meta = with lib; {
    description = "A low-level counterpart to Lua";
    homepage = "http://atomlang.web.app";
